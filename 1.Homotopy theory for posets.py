...
def is_contractible(X):
    if X.has_top() or X.has_bottom(): return True
...
def is_collapsible_aux(X):
	if is_contractible(X): return True
 	if non_collapsibles.has_key(tuple(X.list())): return False
    for x in weak_points(X): 
		S=remove_point(X,x) 
		if not visitados.has_key(tuple(S.list())):
			if is_collapsible(S): return True
...
def is_collapsible(X): #es_lento
    if is_contractible(X): return True
    return min_weak_core(X).cardinality()==1
...
def qc_core(X):
    if len(X.maximal_elements())==1: return X
