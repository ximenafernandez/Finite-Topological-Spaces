...
def presentation_poset(gens,rels):
   ...
   #expanded relations
    ...
    for i in range(len(Rels)):
        ...
        V=V+['c'+str(i+1)+'_'+x+str(j)+'_'+str(k) for x in gens if letters[x]!=0 for j in range(1,4) for k in range(1,letters[x]+1)]+ ['c'+str(i+1)+'_0_'+str(k) for k in range(1,total_letters+1)]
