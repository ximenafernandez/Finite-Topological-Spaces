...
def is_down_O_reduction(X, x):
	if len(X.order_ideal([x])) == 1:
		return False
	for y in X:
		intersection = Set(X.order_ideal([x])).intersection(Set(X.order_ideal([y]))) 
		if not(is_contractible(X.subposet(list(intersection))) or len(intersection) == 0):
			return False
	return True

def is_up_O_reduction(X, x):
	if len(X.order_filter([x])) == 1:
		return False
	for y in X:
...
def middle_core(X):
	...
	for S in Set(height1).subsets(2):
		l = list(S)
		if is_middle_reduction(X, l[0], l[1]) or is_middle_op_reduction(X, l[0], l[1]):
			X = quotient_poset(X, [l[0], l[1]])
